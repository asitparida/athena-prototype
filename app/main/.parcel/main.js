process.env.HMR_PORT=56379;process.env.HMR_HOSTNAME="localhost";// modules are defined as an array
...
},{}],"../api/helpers.ts":[function(require,module,exports) {
"use strict";
...
function getLocalDBFile() {
  const filePath = electron_1.app.getPath('userData') + '/elements.json';
...
exports.getLocalDBFile = getLocalDBFile;
...
const adapter = new FileSync(helpers_1.getLocalDBFile());
...
app.get('/api/mms/', (req, res) => {
  const notes = db.get('mms').value();
  res.json({
    data: notes
  });
});
app.get('/api/mms/list', (req, res) => {
  const mms = db.get('mms').value();
  res.json({
    data: mms.map(m => m.sid)
  });
});
app.post('/api/mms', (req, res) => {
  ...
  const mmsCollection = collection.value();
...
exports.default = app;
},{"./helpers":"../api/helpers.ts"}],"../api/server.ts":[function(require,module,exports) {
"use strict";
...
},{}],"helpers/twilio.ts":[function(require,module,exports) {
"use strict";
  ...
  processMessage(message) {
    if (message && message.sid) {
      if (_.startsWith(message.sid, 'MM')) {
        ...
        client.messages(message.sid).media.list({
          limit: 10
          ...
          client.messages(message.sid).media(m.sid).fetch().then(data => {
            ...
            main_1.SaveMedia(mediaUrl, _.isEmpty(message.body) ? null : message.body, message.sid, message.dateUpdated);
          }, error => console.log);
        }), error => console.log);
      } else {
        // client.messages(message.sid).remove()
        // .then(m => console.log(m.sid));
        main_1.SaveMedia(null, _.isEmpty(message.body) ? null : message.body, message.sid, message.dateUpdated);
      }
    }
  }
...
exports.Twilio = new TwilioManager();
},{"../main":"main.ts"}],"main.ts":[function(require,module,exports) {
"use strict";
...
const twilio_1 = require("./helpers/twilio");
...
app.on('ready', async () => {
  ...
  mainWindow.once('ready-to-show', () => {
    ...
    if (exports.EnableTwilio) {
      twilio_1.Twilio.initialize();
      twilio_1.Twilio.startFetch();
    }
...
async function SaveMedia(mediaUrl, text, sid, modified) {
  const data = {
    mediaUrl,
    text,
    sid,
  ...
  const mmsPostUrl = `http://localhost:${exports.API_PORT}/api/mms/`;
  fetch(mmsPostUrl, {
    method: 'POST',
